...
class Dashboard extends React.Component {
  ...
  componentDidMount() {
    ...
    //if origin was from callback it will refresh the page to get cookie data
    ...
    db.ref('log').once('value').then((membersSnap) => {
        ...
        this.setState({leaderboard:leaderboard.sort(function(a, b) {
      ...
      <div>
        ...
        {this.props.user && (
          <div style={{ padding: "3%" }}>
            ...
            <Grid container className={classes.gridContainerOveride}>
              <Grid item sm={12} md={6} className={classes.gridItem} style={{width:'100%'}}>
                <div className={classes.Card}>
                  ...
                  <div className={classes.cardHeader}>
                    {this.state.competitions && (
                      <i>{this.state.competitions[0].start_date} <a href="https://www.thebluealliance.com/team/2976" target="_blank">View All</a></i>
                    )}
                  </div>
                </div>
              </Grid>
              <Grid item sm={12} md={6} className={classes.gridItem} >
                <div className={classes.Card}>
                    <div className={classes.cardHeader}>Leaderboard | Position: #{this.state.leaderboardPosition}</div>
                  <div className={classes.cardContent} style={{overflow:'auto', height:'99px'}}>
                  {this.state.leaderboard ? 
                        <>
                        {this.state.leaderboard.map((item, index) => {
                        return <p style={{fontSize:'11px'}}>{index+1}) <strong>{item[0].name}</strong> | {this.secToHr(item[0].totalHours)}</p>
                        })}
                        </>
                   : <CircularProgress color="secondary" />}
                  </div>
...
Dashboard.getInitialProps = async function(ctx, query) {
          ...
          function(snapshot) {
            ...
            Object.values(logs).forEach(session => {
                if (session.end0){
                    ...
                    var end = session.end0.split(":");
                    var endSeconds = +end[0] * 60 * 60 + (+end[1] * 60 + +end[2]);
                    hours = hours +  Math.round(((endSeconds - startSeconds) / 3600));
                }
            });
          },
  ...
  if (logs) {
    return { user: user.id, name: name, logs: logs, hours: hours, query:ctx.query.fromCallback };
  }
};
